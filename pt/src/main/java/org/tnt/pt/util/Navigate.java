...
public Navigate() {
   ...
   * @author yuanchen
   */
  public String getOracleQueryPreCondition(){
      String condition = "";
      condition= "select * from (select tmp_tb.*,ROWNUM row_id from ( ";
      return condition;
  }
  /**
   * 获取oracle 分页sql语句后条件
   ...
   * @author yuanchen
   */
  public String getOracleQuerySufCondition(){
      StringBuffer condition = new StringBuffer("");
      condition.append(") tmp_tb where ROWNUM<=")
      		   .append(pageOffset+pageSize)
      		   .append(") where row_id>")
      		   .append((pageId-1)*pageSize);
