...
Calendar.prototype.show = function (targetObject, eventObject , popuControl) {
	...
	this.bindYear();
	this.bindMonth();
	if (targetObject.value.length > 0){
		this.date  = new Date(targetObject.value.toDate(this.patternDelimiter, this.string2DatePattern));
		this.year  = this.date.getFullYear();
		this.month = this.date.getMonth();
	}
	this.changeSelect();
...
document.onclick = function(){
  if(objectId!=window.event.srcElement&&window.event.srcElement!=document.getElementById("goPrevMonthButton")
		  ...
		  &&window.event.srcElement!=document.getElementById("goNextMonthButton")&&window.event.srcElement!=document.getElementById("__calendarTable"))
  {
       document.getElementById("__calendarPanel").style.visibility = "hidden";
  }
  try{
	  //Customer.js中的onclick方法  2个文件一起加载时  调用后加载的onclick方法
	  if(_targetObject!=window.event.srcElement&&window.event.srcElement!=document.getElementById("customerSelect"))
		{
  ...
  try{
	  if(_targetObject!=window.event.srcElement&&window.event.srcElement!=document.getElementById("theDialogBoxDiv"))
		{
			document.getElementById("theDialogBoxDiv").style.visibility = "hidden";
		}   
  }catch(e){}
  try{
	  if(_targetObject!=window.event.srcElement&&window.event.srcElement!=document.getElementById("theButtomDiv"))
		{
			document.getElementById("theDialogBoxDiv").style.visibility = "hidden";
		} 
  }catch(e){}
  try{
	  parent.document.getElementById("theTargetShowDialogBoxIframe").src = "";
  }catch(e){}
 };
document.writeln('<div id="__calendarPanel" style="position:absolute;visibility:hidden;z-index:9999;background-color:#FFFFFF;border:1px solid #666666;width:200px;height:180px;">');
...
__ci.document.writeln('th.year {font-weight:normal;background-color:#7E9BE5;color:#000000;width:24px;}');
__ci.document.writeln('th.theader {font-weight:normal;background-color:#7E9BE5;color:#000000;width:24px;}');
